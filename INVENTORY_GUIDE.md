...
> 최종 갱신: 2026-07-03 (폴더 정리 + 전투력/출석/재화/아바타 반영).
...
- **데이터 권위 = `Inventory.mlua`**(DefaultPlayer 부착). 서버 `_T.slots`(배열) + `_T.equipped`(부위→아이템명) + `_T.unlocked`.
- `@Sync` 문자열 `SlotsCsv`/`EquippedCsv`/`UnlockedCsv`/`LastAttendDate` → 클라 동기화.
- API: `AddItem` / `RemoveItemAt`+`RequestRemoveAt` / `RequestEquip`(부위당 1개 교체) / `RequestUnequip` / `GetEquipped` / `RequestUnlockPart`+`IsUnlocked` / `HasSpaceFor`.
